...
export default function CaseBlog({ activeTag, searchQuery }) {
    // Filtering logic
    const filteredPosts = caseData.filter((post) => {
        const matchesTag = activeTag === "all" || post.tags === activeTag
        const matchesSearch =
                ...
                {filteredPosts.length > 0 ? (
                    filteredPosts.map((post, i) => (
                        ...
                        >
                            ...
                            {/* Tag */}
                            <motion.div className="absolute top-8 right-8 bg-white px-4 py-2 group-hover:text-[#7E1835] text-[#7E1835] font-medium font-garamond rounded-[30px]">
                                {post.tags}
                            </motion.div>
